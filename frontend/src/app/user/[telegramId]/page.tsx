...
function isChallengeData(post: PostData): post is ChallengeData {
  return (
    "frequency" in post &&
    "total_reports" in post &&
    "completed_reports" in post
  );
}
...
export default function UserProfile() {
  ...
  // Фильтр: только свои (исключить подписанные чужие челленджи)
  const searchParams = useSearchParams();
  const pathname = usePathname();
  const router = useRouter();
  const [showOwnOnly, setShowOwnOnly] = useState<boolean>(() => {
    const own = searchParams?.get("own");
    return own === "1";
  });

  // Синхронизация URL при изменении чекбокса
  useEffect(() => {
    if (!router || !pathname) return;
    // избегаем лишней замены URL, если состояние не изменилось
    const currentOwn = searchParams?.get("own") === "1";
    if (currentOwn === showOwnOnly) return;
  ...
  }, [showOwnOnly, router, pathname, searchParams]);
  ...
  const handlePostsUpdate = useCallback(
    (payload: any) => {
      console.log("🔍 handlePostsUpdate called:", {
  ...
  const handleChallengesUpdate = useCallback(
    (payload: any) => {
      // Для всех событий перезагружаем весь список для избежания дублирования
      loadUserData();
    },
  ...
  const filteredPosts = useMemo(() => {
    if (isOwn) {
      // Для владельца профиля: по умолчанию показываем свои (обещания и челленджи) + подписанные челленджи
      // При включенном фильтре исключаем подписанные чужие челленджи
      if (!showOwnOnly) return allPosts;
      const subscribedIds = new Set(subscribedChallenges.map((c) => c.id));
      return allPosts.filter((post) => {
        if (isChallengeData(post)) {
          const isSubscribedForeign =
            subscribedIds.has(post.id) && post.user_id !== telegramId;
          return !isSubscribedForeign;
        }
        return true;
      });
    }
  ...
  }, [allPosts, isOwn, ctxId, showOwnOnly, subscribedChallenges, telegramId]);
  ...
  const { handleDeleteChallenge, handleUpdateChallenge } = useChallengeApi(
    updatePosts,
    setError
  );
  ...
  const [promiseCreators, setPromiseCreators] = useState<
    Record<number, UserData>
  >({});
  // --- Новый блок: загрузка информации о получателях обещаний ---
  const [promiseRecipients, setPromiseRecipients] = useState<
    Record<number, UserData>
  >({});
  ...
  const loadPromiseRecipients = async (promises: PromiseData[]) => {
    ...
    try {
      ...
      if (!error && recipients) {
        const mapped = recipients.reduce(
          (acc, recipient) => {
  ...
  const loadSubscribedChallenges = async () => {
    ...
    try {
      ...
      const challenges =
        (participants
          ?.map((p) => {
            const challenge = Array.isArray(p.challenges)
              ? p.challenges[0]
              : p.challenges;
            if (!challenge) return null;
            let owner = Array.isArray(challenge.owner)
              ? challenge.owner[0]
              : challenge.owner;
      ...
      setAllPosts((prev) => {
        ...
        const newChallenges = challenges.filter(
          (challenge) => challenge && !existingIds.has(challenge.id)
        );
        if (newChallenges.length === 0) return prev;
        const updated = [...prev, ...newChallenges] as (
          | PromiseData
          | ChallengeWithOwner
        )[];
        return updated.sort(
          (a, b) =>
            new Date((b as ChallengeWithOwner).created_at).getTime() -
            new Date((a as ChallengeWithOwner).created_at).getTime()
        );
      });
    } catch (error) {
      console.error("Error loading subscribed challenges:", error);
  ...
  useEffect(() => {
    if (userData) {
      console.log("📊 User stats updated:", {
        ...
        total_done:
          (userData.promises_done || 0) + (userData.challenges_done || 0),
      });
    }
  }, [
    userData?.promises,
    userData?.promises_done,
    userData?.challenges,
    userData?.challenges_done,
  ]);
                    ...
                    isPromiseData(post) ? (
                        ...
                        {(() => {
                          ...
                          if (post.recipient_id) {
                            ...
                            if (post.recipient_id === telegramId) {
                              ...
                              const recipient =
                                promiseRecipients[post.recipient_id];
                              if (recipient) {
                                recipientName =
                              ...
                              avatarUrl={
                                post.user_id === telegramId
                                  ...
                                  : promiseCreators[post.user_id]
                                      ?.avatar_img_url || defaultAvatarImg
                              }
                              userId={post.user_id}
                              userCtxId={ctxId || 0}
                              userName={
                                post.user_id === telegramId
                                  ? fullName
                                  : promiseCreators[post.user_id]
                                    ? `${promiseCreators[post.user_id].first_name || ""} ${promiseCreators[post.user_id].last_name || ""}`.trim() ||
                                      `@${post.user_id}`
                                    : `@${post.user_id}`
                              }
                      ...
                      >
                        <ChallengeView
                          ...
                          isProfilePage
                          onStart={() =>
                            handleUpdateChallenge(post.id, telegramId, "start")
                          }
                          onCheckDay={() =>
                          ...
                          onFinish={() =>
                            handleUpdateChallenge(post.id, telegramId, "finish")
                          }
                        />
                      </motion.div>
                    )
