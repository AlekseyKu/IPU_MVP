...
export default function ListPage() {
  ...
  const { t } = useLanguage();
  const { posts, users, subscriptions, isLoading, hasMore, loadMorePosts } =
    usePublicPosts(currentUserId, showSubscribedOnly);
  ...
  // фильтрация подписок выполняется внутри usePublicPosts при включенном чекбоксе
                    ...
                    {filteredPosts.map((post) => {
                      ...
                      if (isPromise && (post as PromiseData).recipient_id) {
                        const recipient = users[
                          (post as PromiseData).recipient_id!
                        ] || {
                          first_name: "",
                          last_name: "",
                          username: "",
